...
"""Install the bundled ledidi Agent Skill for Claude Code.

This module exposes the ``ledidi-install-skills`` console script. It copies the
Agent Skill that ships inside the installed package into a location where Claude
Code discovers skills, so that a coding agent gains ledidi-specific guidance (the
design objective, the oracle contract, and the footguns that silently produce
wrong designs) in every project, not just this repository.
...
Run ``ledidi-install-skills`` to install, or ``--print-path`` to see where the
...
def install_skill(dest: Path | None = None, force: bool = False) -> Path:
	"""Copy the bundled ledidi skill into a Claude Code skills directory.
	...
	dest: pathlib.Path or None, optional
		...
		``~/.claude/skills/ledidi``. Default is None.
...
def main(argv: list[str] | None = None) -> int:
	"""Command-line entry point for ``ledidi-install-skills``.
	...
	parser = argparse.ArgumentParser(
		prog="ledidi-install-skills",
		description="Install the ledidi Agent Skill for Claude Code.")
	parser.add_argument("--dest", type=Path, default=None,
		help="Destination directory (default: ~/.claude/skills/ledidi).")
	...
	print("Installed ledidi skill to {}".format(dest))
	print("It will be available to Claude Code in your next session.")
	print("ledidi depends on tangermeme; install its skill too with "
