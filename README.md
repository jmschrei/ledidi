...
The example below uses a real BPNet oracle that predicts GATA2 binding. It requires `bpnetlite` (`pip install bpnetlite`) and downloads a small (~0.5 MB) model from Zenodo on first run, and the design takes roughly half a minute on a CPU. For a dependency-free example that runs anywhere with no downloads, see the [Quickstart](#quickstart).
...
X_hat = ledidi(model, X, y_bar, device="cpu", random_state=0)  # ~30s on CPU
```
