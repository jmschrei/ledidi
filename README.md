...
**Software.** Python >= 3.10 and PyTorch >= 2.0. Installing Ledidi pulls in [tangermeme](https://github.com/jmschrei/tangermeme) (>= 1.3.0), `numpy`, and `matplotlib` automatically, and `tangermeme` brings a scientific stack of its own (36 packages in a clean environment); nothing else needs installing to design sequences. The tutorials additionally use whichever oracle package they demonstrate (`bpnetlite`, `enformer-pytorch`, `boda`), but Tutorials 0 and 8 run with no extra packages and no downloads.

**Hardware.** Ledidi runs on a CPU or a CUDA GPU, and there is no minimum GPU — the toy example below finishes in about three seconds on a single CPU thread. Note that `ledidi` defaults to `device='cuda'`, so on a machine without a GPU you must pass `device='cpu'` explicitly. Memory is dominated by the oracle rather than by Ledidi: a BPNet-scale design over a 2114 bp sequence peaks at 232 MB of GPU memory at the default `batch_size=16`, scaling roughly linearly in `batch_size`. The same design takes 0.8s on a GPU against roughly 30s on a single CPU thread (8s if PyTorch is given eight). Large long-context oracles are the case that actually strains a card.
